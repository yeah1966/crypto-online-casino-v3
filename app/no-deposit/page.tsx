...
export default function NoDepositPage() {
  ...
  return (
    <main className="relative max-w-4xl mx-auto px-4 py-4 pt-6 text-white">
      ...
      {/* MAIN TRANSPARENT CONTAINER LIKE WILD VEGAS */}
      <div className="relative z-10 max-w-3xl mx-auto rounded-3xl bg-black/70 shadow-2xl p-8 space-y-10 border border-purple-700/40 mt-2">
        ...
        <section className="animate-fade-in-up">
          <div className="flex items-center justify-center gap-4 mb-3">
            ...
            <h1 className="text-4xl sm:text-5xl font-extrabold drop-shadow-[0_0_25px_#4ade80] uppercase text-green-300 neon-glow animate-glow tracking-tight">No Deposit Bonuses</h1>
          </div>
          <p className="text-lg text-white/80 mt-2 mb-1 max-w-2xl mx-auto flex items-center gap-2 justify-center">
        ...
        <section className="animate-fade-in-up">
          <div className="grid gap-8 sm:grid-cols-2 md:grid-cols-3">
            {filtered.length === 0 && (
              <div className="col-span-full text-center text-white/80 text-lg">No no deposit bonus info available at this time.</div>
            )}
            {filtered.map(casino => (
              <div key={casino.slug} className={`rounded-2xl border-2 ${casino.noDepositBonus ? 'border-green-400' : 'border-yellow-300'} bg-black/80 text-white p-5 shadow-xl flex flex-col items-center hover:scale-105 transition-all duration-300 group`}>
                ...
                <div className="text-white/80 text-center mb-2">
                  {casino.noDepositBonus ? 'Real No Deposit Bonus' : 'No real no deposit bonus'}
                </div>
                <div className="text-white/60 text-center mb-2 text-sm">{casino.noDepositNote}</div>
